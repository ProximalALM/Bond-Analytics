...
yield_curve = term_structure(bond_data, bond_info, bond_calendar, 30, 12, 2008, day_count, convention, 'Piecewise Log Cubin Discount')
...
for k in range(len(today_set)):
    ...
    bond_yield = fixed_rate_bond.bondYield(clean_price, day_count, compounding, compounding_frequency, today_set[k])

    interest = InterestRate(bond_yield, day_count, compounding, compounding_frequency)
    ...
    result_array.append([name[7], bond_datee[k], clean_price, dirty_price, bond_yield, accrued, npv,
...
df_result.to_csv(outputPath + 'result_080010_actual.csv', index = False)
